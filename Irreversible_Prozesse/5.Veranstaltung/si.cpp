...
*/
		...
		void infect(int N,double pinf) 
		{  
		  for(int i=0;i<N;i++)
		  {
		    if(nodes[i].gesundheitbefore==inf)
		    {
		      for(int j=1;j<nodes[i].get_numneigh();j++)
		      {
			int k = gsl_rng_uniform(rng);
			if(k<pinf && nodes[j].gesundheitbefore==sus)
			{
			  nodes[j].gesundheitnow=inf;
			}
			else
			{
			  nodes[j].gesundheitnow=nodes[j].gesundheitbefore;
			}  
		      }
		    }
		  }
