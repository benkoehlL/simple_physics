...
int main()
{
  ...
  if(Test==1)
  {
	...
	for(int i=0;i<3;i++)
	{
	  alpha1[t*3+i]=B1[O1[t+1]*3]*A1[i]*alpha1[(t+1)*3]+B1[O1[t+1]*3+1]*A1[3+i]*alpha1[(t+1)*3+1]+B1[O1[t+1]*3+2]*A1[6+i]*alpha1[(t+1)*3+2];
	}
	cout << alpha1[t*3] << '\t' << alpha1[t*3+1] << '\t' << alpha1[t*3+2] << '\n';
    }
    cout << '\n' <<  log(p1[0]*alpha1[0]*B1[O1[0]]+p1[1]*alpha1[1]*B1[O1[0]+1]+p1[2]*alpha1[2]*B1[O1[0]+2]) << '\n';
  }
  else
  {
    alpha2[0]=0;
